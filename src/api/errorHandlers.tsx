...
export async function handleGraphQlApiError(error: Error): Promise<void> {
  if (error instanceof UsernameTakenError) message.error('That username is taken. Try another.');
    ...
    message.error('There must be at least one action, and each must be unique.');
  else if (error instanceof InvalidInvitedChatError) message.error('That group chat is no longer accepting invites.');
  else if (error instanceof InvalidPollError)
    message.error('Polls must have at least two options, all of which are unique.');
  else if (error instanceof IncorrectPasswordError) message.error('Incorrect password.');
  else if (error instanceof InvalidInviteCodeError) message.error('That group chat was deleted.');
  else {
...
export async function handleRestApiError(error: Error): Promise<void> {
  if (error instanceof InvalidPicError) message.error('The picture must be a PNG or JPEG file not exceeding 5 MB.');
  if (error instanceof InvalidAudioError) message.error('The audio must be an MP3 or MP4 not exceeding 5 MiB.');
  if (error instanceof InvalidVideoError) message.error('The video must be an MP4 not exceeding 5 MiB.');
  if (error instanceof InvalidDocError) message.error("The document mustn't exceeding 5 MiB.");
  else {
    await handleCommonApiError(error);
    return;
  }
