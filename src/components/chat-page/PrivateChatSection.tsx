...
export default function PrivateChatSection({
                                             chat,
                                           }: PrivateChatSectionProps): ReactElement {
  return (
    <Layout>
      <Layout.Header>
...
function OnlineStatusSection({userId}: OnlineStatusSectionProps): ReactElement {
  const onlineStatus = useSelector((state: RootState) =>
    OnlineStatusesSlice.select(state, userId)
  );
...
function TypingStatusSection({
                               userId,
                               chatId,
                             }: TypingStatusSectionProps): ReactElement {
  const dispatch = useDispatch();
  dispatch(TypingStatusesSlice.fetchStatuses());
  const isTyping = useSelector((state: RootState) =>
    TypingStatusesSlice.selectIsTyping(state, userId, chatId)
  );
...
function ProfilePic({userId}: ProfilePicProps): ReactElement {
  ...
  const error = useSelector((state: RootState) =>
    PicsSlice.selectError(state, 'PROFILE_PIC', userId)
  );
  dispatch(PicsSlice.fetchPic({id: userId, type: 'PROFILE_PIC'}));
  if (error instanceof NonexistentUserIdError)
    message
      .warning('That user just deleted their account.')
      .then(() => setContent(<Empty style={{padding: 16}}/>));
