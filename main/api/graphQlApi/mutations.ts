...
export namespace Mutations {
    ...
    /**
     * Used when the user wants to reset their password because they forgot it. Sends a password reset email to the
    ...
    /**
     * Updates the user's account. Only the non-`null` fields will be updated. None of the updates will take place if
     * even one of the fields were invalid. If the user updates their email address to something other than their
     * current address, you must log them out because the current access token will be invalid until they verify their
     * new email address.
     *
     * If the user updates their email address, they'll be required to verify it before their next login via an email
     * which is sent to it. This means they'll be locked out of their account if they provide an invalid address, and
     * will have to contact the service's admin to correctly update their address. You could prevent this mistake by
     * asking them to confirm their address. For example, a UI could require the user to enter their email address twice
     * if they're updating it, and only allow the update to take place if both the entered addresses match.
